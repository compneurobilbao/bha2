...
def connectome_average(slist):
    ...
    for i, sub in enumerate(slist):
        print(i)
        sc = np.array(pd.read_csv('sc_matrices/' + sub +
                      '_anat_probabilistic_connectome.csv', delimiter=' ', header=None))
        fc = np.corrcoef(np.transpose(
            np.genfromtxt('timeseries/ts_' + sub + '.txt')))
        scmod = np.log10(sc+1)
        fc_all.append(fc)
...
def mod_similarity(fc, sc, T):
    ...
    for i in range(1, np.max(T)+1):
        ...
        fc_mod_m.append(fc.iloc[rois_in_clust]
                        [rois_in_clust].to_numpy().mean())
        sc_mod_m.append(sc.iloc[rois_in_clust]
                        [rois_in_clust].to_numpy().mean())
...
def calc_connfeat(fc, sc, T, lvl):
    ...
    for i in range(1, np.max(T)+1):
        rois_in_clust = np.where(T == i)[0]
        ext_rois = np.setdiff1d(
            np.array([i for i in range(len(T))]), rois_in_clust)
        ...
        if len(rois_in_clust) > 1:
            ...
            sc_int = (sc.iloc[rois_in_clust]
                      [rois_in_clust].to_numpy().sum())/len(rois_in_clust)
            sc_out = (sc.iloc[rois_in_clust]
                      [ext_rois].to_numpy().sum())/len(rois_in_clust)
...
def generate_population_features(slist, g, num_clust_init, num_clust_end, scm, fcm):
    ...
    for num_clust in range(num_clust_init, num_clust_end+1, 1):
        ...
        for i, sub in enumerate(slist):
            print('gamma = ' + str(g) + ', lvl = ' +
                  str(num_clust) + ', snumber = ' + str(i))
            sc = np.array(pd.read_csv('sc_matrices/' + sub +
                          '_anat_probabilistic_connectome.csv', delimiter=' ', header=None))
            fc = np.corrcoef(np.transpose(
                np.genfromtxt('timeseries/ts_' + sub + '.txt')))
            scmod = np.log10(sc+1)
            subfeatures, subfeat_names, feat_dict = calc_connfeat(
