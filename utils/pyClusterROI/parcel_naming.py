...
def main():
    ...
    print(parcel_vals)
    print("%s called with %s, %s, %s" %(sys.argv[0],parcel_filename, parcel_outname,",".join([str(i) for i in parcel_vals])))
    ...
    if len(parcel_vals) != np.shape(parcels_img)[3]:
        print("Length of parcel values (%d) != number of parcel images (%d)"%( len(parcel_vals),np.shape(parcels_img)[3]))
