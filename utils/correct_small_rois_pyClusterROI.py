...
def small_roi_correction(lp_path, area, vol_des, min_vol):
    (
        ...
        image_roi_volumes,
        image_mean_roi_vol,
    ) = select_rois_from_lobe_partitions(lp_path, area)
    size_near_desired = np.argmin(np.abs(np.array(image_mean_roi_vol) - vol_des))
    image_roi_vol_selected = image_roi_volumes[size_near_desired]
    image_selected_orig = nib.load(image_names[size_near_desired])
    small_rois = image_roi_labels[size_near_desired][
        np.where(image_roi_vol_selected < min_vol)[0]
    ]

    vol_to_correct = image_selected_orig.get_fdata()
    if len(small_rois) > 0:
        for sr in small_rois:
            sr_loc = np.where(vol_to_correct == sr, 1, 0).astype(bool)
            sr_dil_loc = binary_dilation(sr_loc, iterations=1)
            neighbours = np.setdiff1d(
                np.unique(vol_to_correct[sr_dil_loc]), [0, sr]
            ).astype(int)
            if len(neighbours) == 0:
            ...
            if len(neighbours) > 0:
                size_neighbours = image_roi_vol_selected[
                    np.where(np.in1d(image_roi_labels[size_near_desired], neighbours))[
                        0
                    ]
                ]
...
def get_roi_anatomical_description(atlas):
    ...
    for area in brain_areas:
        for roi_label in atlas_rois:
            area_mask_L = nib.load(
                os.path.join(
                    "/workspaces",
                    "bha2",
                    "brain_templates",
                    "lobule_masks",
                    "masks_splitted_in_hemispheres",
                    area + "_L_mask.nii.gz",
                )
            )
            area_mask_L_data = area_mask_L.get_fdata()
            area_mask_R = nib.load(
                os.path.join(
                    "/workspaces",
                    "bha2",
                    "brain_templates",
            ...
            coords.loc[roi_label, area + "_L"] = image_overlaps(
                roi_mask, area_mask_L_data
            )
            coords.loc[roi_label, area + "_R"] = image_overlaps(
                roi_mask, area_mask_R_data
            )
...
for b_area in brain_areas:
    vol_for_small_rois_correction, image_selected = small_roi_correction(
        LOBE_PARTITION_PATH, b_area, volume_per_roi_desired, min_volume_per_roi
    )
    new_roi_labels = np.setdiff1d(np.unique(vol_for_small_rois_correction), 0).astype(
        int
    )
    for roi in new_roi_labels:
        template_empty[vol_for_small_rois_correction == roi] = FINAL_ROI_LABEL_COUNTER
        FINAL_ROI_LABEL_COUNTER += 1

nib.save(
    nib.Nifti1Image(template_empty, image_selected.affine),
    os.path.join(
        "/workspaces",
        "bha2",
        "data",
...
atlas_coords = get_roi_anatomical_description(
    nib.Nifti1Image(template_empty, image_selected.affine)
)
